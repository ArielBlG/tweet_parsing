...
def anonymize_data(row: pd.Series,
    ...
    def process_nested_tweet(tweet):
        ...
        if tweet.get('new_full_entities', {}).get('user_mentions'):
            for mention in tweet['new_full_entities']['user_mentions']:
                _hash = get_hash(mention['name'])
                value = (mention['name'], mention['screen_name'])
